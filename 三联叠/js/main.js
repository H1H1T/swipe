$(function(){
	var $box=$('.main_banner ul').children('li');
	var len=$('.main_banner ul').children('li').length;
	var index_2=0;
	var timer=800;
	var intervaltimer=0;
	var isMoving=false;
		...
		if (isMoving==false){
			...
			if(slideMode){			//slideMode为1(true)，执行slide(1)，上一张
				...
				if(index_2+2>=len){								//index_2								3		4
					hidden=$box.eq(index_2+2-len);//										0		1
				}else{													//index_2		0		1		2
					hidden=$box.eq(index_2+2);//						2		3		4
				}
				prev.css("z-index","1");
	...
	if(len>3){
		...
		$(".btn_list span").click(function(event){
			...
			slide(flag);
			intervaltimer=setInterval(slide,30000);//自动轮播
		...
		$(".main_banner_box li").on("mousemove",function(){
			if($(this).css("width")=="670px"){//鼠标移入为当前正中显示的图片li，则清除定时器
				...
				clearInterval(intervaltimer);
				intervaltimer=null;
				intervaltimer=setInterval(slide,200);
		});
